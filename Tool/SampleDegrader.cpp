 ...
 */
void outputReplacementsYAML(const clang::tooling::TranslationUnitReplacements &replacements){
    ...
    yaml_out << const_cast<clang::tooling::TranslationUnitReplacements &>(replacements);
...
int main(int argc, const char **argv) {
    ...
    std::map<std::string, clang::tooling::TranslationUnitReplacements> tu_replacements_map = 
        nullptr_degrader.getTUReplacementsMap();
    for(std::map<std::string,clang::tooling::TranslationUnitReplacements>::iterator 
            it=tu_replacements_map.begin(), end=tu_replacements_map.end();
            it!=end; it++){
